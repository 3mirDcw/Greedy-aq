    ...
    guild.fetchInvites().then(invites => {
        ...
        if (invite.inviter) { 
            ...
            if(fake){
                ...
                fake = db.add(`invites.${invite.inviter.id}.fake`, 1);
            }
            else{
                total = db.add(`invites.${invite.inviter.id}.total`, 1);
        ...
        if(channel){
       channel.send(`${member} Adlı Kişi Sunucuya Katıldı **Davet Eden kişi :** ${invite.inviter.tag} (**${total + bonus}** Davet! :white_check_mark:)`)
        }
    }).catch();
});
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
     if(channel){
        let user = client.users.cache.get(data.inviter)
     	channel.send(`${member.user.tag} Sunucudan Ayrıldı **Kişiyi Davet Eden:** ${user.tag} (**${Number(total) + Number(bonus)}** Davet! :x:)`)
     }
});
