...
//pythonic
    ...
    if(!victim) return message.reply("Lütfen Bir Kişinin Adını Yazınız");//pythonic//pythonic
    victim = message.guild.member(victim);//pythonic
    if(!victim) return message.reply("Adını Yazdığınız Kişi Sunucuda Bulunmamaktadır!");//pythonic

    var num = Number(args[1]);//pythonic
    if(isNaN(num)) return message.reply("Lütfen Bonus Olucak Sayıyı Giriniz!");//pythonic
