...
exports.run = async(bot, message, args) => {
    ...
    if (tomute.hasPermission("MANAGE_MESSAGES")) return message.reply(" Geçici olarak susturmaya çalıştığınız kişi yetkili veya bot'un yetkisi belirttiğiniz kişiyi geçici olarak susturmaya yetkisi bulunmuyor!");
    let muterole = message.guild.roles.find(r => r.name === "Greedy_Susturulmuş");

    if (!muterole) {
        try {
            muterole = await message.guild.createRole({
                name: "Greedy_Susturulmuş",
                color: "#818386",
                permissions: []
            })
